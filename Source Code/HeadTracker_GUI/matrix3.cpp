...
// this is based on http://gentlenav.googlecode.com/files/EulerAngles.pdf
template <typename T>
void Matrix3<T>::to_euler(double *roll, double *pitch, double *yaw)
{
	if (pitch != NULL) {
		*pitch = -safe_asin(c.x);
	}
...
template void Matrix3<double>::rotation(enum Rotation);
...
template void Matrix3<double>::to_euler(double *roll, double *pitch, double *yaw);
