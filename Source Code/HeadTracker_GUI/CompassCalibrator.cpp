...
uint32_t millis(){
	/*
		...
		DateTime^ utc = gcnew DateTime(1970, 1, 1);

		double ms = localDate.TotalMilliseconds  -  utc->TotalMilliseconds;
          return  (uint32_t) ms;
	*/
	return GetTickCount();
}

void CompassCalibrator::start(bool retry, double delay, uint16_t offset_max) {
    if(running()) {
        return;
    }
...
double CompassCalibrator::get_completion_percent() const {
    ...
    switch(_status) {
        ...
        case COMPASS_CAL_RUNNING_STEP_TWO:
            return 33.3f + 65.7f*((double)(_samples_collected-_samples_thinned)/(COMPASS_CAL_NUM_SAMPLES-_samples_thinned));
...
bool CompassCalibrator::fit_acceptable() {
    if( !isnan(_fitness) &&
        ...
        fabs(_params.offset.x) < _offset_max &&
        fabs(_params.offset.y) < _offset_max &&
        fabs(_params.offset.z) < _offset_max &&
        _params.diag.x > 0.2f && _params.diag.x < 5.0f &&
        _params.diag.y > 0.2f && _params.diag.y < 5.0f &&
        _params.diag.z > 0.2f && _params.diag.z < 5.0f &&
        fabs(_params.offdiag.x) <  1.0f &&      //absolute of sine/cosine output cannot be greater than 1
        fabs(_params.offdiag.y) <  1.0f &&
        fabs(_params.offdiag.z) <  1.0f ){

            return _fitness <= _tolerance*_tolerance;
        }
    return false;
}
...
bool CompassCalibrator::accept_sample(const Vector3f& sample)
{
    ...
    static const double a = (4.0f * M_PI / (3.0f * faces)) + M_PI / 3.0f;
    static const double theta = 0.5f * acos(cos(a) / (1.0f - cos(a)));

    if(_sample_buffer == nullptr) {
        return false;
    }

    double min_distance = _params.radius * 2*sin(theta/2);

    for (uint16_t i = 0; i<_samples_collected; i++){
        double distance = (sample - _sample_buffer[i].get()).length();
...
double CompassCalibrator::calc_residual(const Vector3f& sample, const param_t& params) const {
    Matrix3f softiron(
...
double CompassCalibrator::calc_mean_squared_residuals() const
{
    return calc_mean_squared_residuals(_params);
}

double CompassCalibrator::calc_mean_squared_residuals(const param_t& params) const
{
    ...
    for(uint16_t i=0; i < _samples_collected; i++){
        ...
        double resid = calc_residual(sample, params);
        sum += (resid)*(resid);
    }
...
void CompassCalibrator::calc_sphere_jacob(const Vector3f& sample, const param_t& params, double* ret) const{
    ...
    double A =  (diag.x    * (sample.x + offset.x)) + (offdiag.x * (sample.y + offset.y)) + (offdiag.y * (sample.z + offset.z));
    double B =  (offdiag.x * (sample.x + offset.x)) + (diag.y    * (sample.y + offset.y)) + (offdiag.z * (sample.z + offset.z));
    double C =  (offdiag.y * (sample.x + offset.x)) + (offdiag.z * (sample.y + offset.y)) + (diag.z    * (sample.z + offset.z));
    double length = (softiron*(sample+offset)).length();
...
void CompassCalibrator::run_sphere_fit()
{
    ...
    const double lma_damping = 10.0f;

    double fitness = _fitness;
    double fit1, fit2;
    param_t fit1_params, fit2_params;
    fit1_params = fit2_params = _params;

    double JTJ[COMPASS_CAL_NUM_SPHERE_PARAMS*COMPASS_CAL_NUM_SPHERE_PARAMS] = { };
    double JTJ2[COMPASS_CAL_NUM_SPHERE_PARAMS*COMPASS_CAL_NUM_SPHERE_PARAMS] = { };
    double JTFI[COMPASS_CAL_NUM_SPHERE_PARAMS] = { };

    // Gauss Newton Part common for all kind of extensions including LM
    for(uint16_t k = 0; k<_samples_collected; k++) {
        Vector3f sample = _sample_buffer[k].get();

        double sphere_jacob[COMPASS_CAL_NUM_SPHERE_PARAMS];
...
void CompassCalibrator::calc_ellipsoid_jacob(const Vector3f& sample, const param_t& params, double* ret) const{
    const Vector3f &offset = params.offset;
    const Vector3f &diag = params.diag;
    const Vector3f &offdiag = params.offdiag;
    Matrix3f softiron(
...
void CompassCalibrator::run_ellipsoid_fit()
{
    ...
    double JTJ[COMPASS_CAL_NUM_ELLIPSOID_PARAMS*COMPASS_CAL_NUM_ELLIPSOID_PARAMS] = { };
    double JTJ2[COMPASS_CAL_NUM_ELLIPSOID_PARAMS*COMPASS_CAL_NUM_ELLIPSOID_PARAMS] = { };
    double JTFI[COMPASS_CAL_NUM_ELLIPSOID_PARAMS] = { };

    // Gauss Newton Part common for all kind of extensions including LM
    for(uint16_t k = 0; k<_samples_collected; k++) {
        Vector3f sample = _sample_buffer[k].get();

        double ellipsoid_jacob[COMPASS_CAL_NUM_ELLIPSOID_PARAMS];
...
#define COMPASS_CAL_SAMPLE_SCALE_TO_FIXED(__X) ((int16_t)constrain_float(round(__X*8.0f), INT16_MIN, INT16_MAX))
